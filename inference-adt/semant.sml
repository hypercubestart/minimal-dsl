...
struct
  ...
  |   subst(T.Constructor(adt, field), tenv) = T.Constructor(subst(adt, tenv), subst(field, tenv))
  |   subst(T.Tuple(tys), tenv) = T.Tuple(map (fn x => subst(x, tenv)) tys)
  ...
  |   occursin(T.Constructor(ty, tys), meta) = false (*DOESNT HANDLE RECURSIVE ADT*)
  |   occursin(T.Tuple(tys), meta) = foldl (fn(ty, bool) => bool orelse occursin(ty, meta)) false tys
  ...
  |   unify(T.App(T.ADT(f1), tyvars1), T.App(T.ADT(f2), tyvars2)) = 
        if f1 <> f2 then print("adt cons cannot unify") else unifylist(tyvars1, tyvars2)
  ...
  |   unify(T.Constructor(ty1, f1), T.Constructor(ty2, f2)) = (unify(ty1, ty2); unify(f1, f2))
  |   unify(T.Tuple(tys1), T.Tuple(tys2)) = unifylist(tys1, tys2)
  ...
  |   getMetaVars(T.App(tycon, tys), ht) =
      (
        ...
        T.App(T.ADT(symbols), tys) => () (*DOESNT HANDLE RECURSIVE RECORDS*)
  ...
  fun transExp(venv: venv, tenv) = 
    let fun trexp(A.VarExp(var)) = trvar(var)
          ...
          | trexp(A.OpExp{left, oper, right}) =
              ...
              unify((trexp right), T.App(T.Int, [])); 
              T.App(T.Int, [])) *)
          | trexp(A.ConstructorExp{con, args}) = 
              let
                val SOME(E.TyEntry{ty}) = S.look(tenv, con)
                val ty = instantiate(ty)
                val meta = T.Meta(newmetavar())
                val tys = map (fn x => trexp(x)) args
              in
                (unify(ty, T.Constructor(meta, T.Tuple(tys))); meta)
              end
  ...
  and tydec(venv, tenv, A.ADTDec(symbol, tysymbols, cons)) = 
    let
      val unique = ref(())
      ...
      val fieldtys = map (fn x => T.Tuple(map (fn x=>transty(tenv', x)) (#2 x))) cons
      val constructor_names = map #1 cons
      val adtTycon = T.Unique(ref(SOME(T.TyFun(tyv, T.App(T.ADT(constructor_names), fieldtys)))), unique)
      val tenv'' = S.enter(tenv, symbol, E.TyConEntry{tycon=adtTycon})
      val recordTy = T.App(adtTycon, map (fn x => T.Var(x)) tyv)
      val tenv'' = foldl (fn((fname, fty), tenv) => S.enter(tenv, fname, E.TyEntry{ty=T.Poly(tyv, T.Constructor(recordTy, fty))})) tenv'' (zip(constructor_names, fieldtys))
  ...
  |   transty(tenv, A.TupleTy(tys)) = T.Tuple(map (fn x => transty(tenv, x)) tys)
end